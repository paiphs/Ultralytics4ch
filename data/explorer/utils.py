...
def plot_query_result(similar_set, plot_labels=True):
    ...
    print('data/explorer/utils.py IS RUNNING')
    plot_size = 640
    imgs, batch_idx, plot_boxes, plot_masks, plot_kpts = [], [], [], [], []
    for i, imf in enumerate(images):
        im = cv2.imread(imf, cv2.IMREAD_UNCHANGED)
        print('HERE AT READING in data/explorer/utils.py', im.shape)
        im = cv2.cvtColor(im, cv2.COLOR_BGRA2RGB)#im = cv2.cvtColor(im, cv2.COLOR_BGR2RGB)
